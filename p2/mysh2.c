...
void sh_ext_cmd(char *command, char **args, int argct)
{
    //printf("External command entered:%s\n", command);
    ...
    while(token != NULL && !found)
    {
        ...
        //        printf("path: -->%s<--\n",path);
        //       printf("fullcmd: -->%s<--\n",fullcmd);
        //Copy token from strtok into path array
        strcpy(fullcmd,token);


        //      printf("Path from strtok: -->%s<--\n",fullcmd);

        //Append a / to the end of the path
        strcat(fullcmd,"/");
        ...
        //    printf("Path after appending command: -->%s<--\n",fullcmd);

        //Use access() to check if program exists at that path
        if(access(fullcmd,X_OK) == 0) 
        {
            //printf("Program found at path :%s\n",fullcmd);
            found = 1;
        }
        else 
        {
    ...
    //Command now found, held in array fullcmd
    ...
    if(child_pid==0)
    {
        char* arr[] = {fullcmd, *args,'\0'};
        int i = execv(fullcmd, arr);
        //printf("execv called, i = %d \n", i);
    }

    // Parent wait for child.
     do {
       tpid = wait(&child_status);
       if(tpid != child_pid) exit(1);
...
int main()
{
    ...
    char validcommands[][CMD_LEN] = {"echo", "PS1", "cat", "cp", "rm", "mkdir", "rmdir", "touch","myls","exit"};
    ...
    do 
    {
        ...
        //If CTRL-D entered, exit program 
        if(command[0] == '\0')
        {
        ...
        switch(sw)
        {
            ...
            case -2:
                ...
                //invalid command
            case -1:
                //printf("%s:Not built in command, searching path. \n",command );
                sh_ext_cmd(command, kwargs, argct);
                continue;
                //echo
            case 0:
